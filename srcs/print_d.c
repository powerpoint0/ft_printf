...
int ft_count_len(int len,char *str, t_prn *prn)
{
	...
	//sign = (prn->type == 'p')? 2 : 0;
	if(str[0]== '0' && prn->type == 'o' && prn->precision == 0 && prn->fl_sharp )
		prn->fl_sharp = 1;
	else if ((str[0]== '0')|| (prn->precision >len && prn->fl_sharp && prn->type == 'o'))
		prn->fl_sharp = 0;
	if (prn->precision == 0 && str[0]== '0')
		ft_bzero(str,27);
	len = ft_strlen(str);
	if (prn->sign == -1 || prn->fl_space || (prn->fl_plus &&(ft_strchr("ouxXp", prn->type) == 0)))
		sign = 1;
	else sign = 0;
	if (prn->fl_sharp)
	{
